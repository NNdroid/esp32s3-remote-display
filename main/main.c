...
// 非同步優化快取：讓 Core 0 代為跑沉重的 API，Core 1 解碼達到 0 延自由阻礙
...
// 💥 真·真透明背景高速画字工具函数：只重寫帶有點陣字模的像素點，完美保留 PSRAM 原圖背景背景
void draw_string_8x8_transparent_fast(uint16_t *frame_buffer, int x, int y, const char *str, uint16_t color) {
    ...
    while (*str) {
        ...
        if (cursor_x + 8 > LCD_WIDTH) break;

        uint16_t *char_base_ptr = frame_buffer + (y * LCD_WIDTH) + cursor_x;

        for (int i = 0; i < 8; i++) {
            if (y + i < 0 || y + i >= LCD_HEIGHT) continue;
            uint8_t line_data = font8x8[font_idx][i];
            uint16_t *pixel_ptr = char_base_ptr + (i * LCD_WIDTH);

            if (line_data & 0x80) pixel_ptr[0] = color;
            if (line_data & 0x40) pixel_ptr[1] = color;
            if (line_data & 0x20) pixel_ptr[2] = color;
            if (line_data & 0x10) pixel_ptr[3] = color;
            if (line_data & 0x08) pixel_ptr[4] = color;
            if (line_data & 0x04) pixel_ptr[5] = color;
            if (line_data & 0x02) pixel_ptr[6] = color;
            if (line_data & 0x01) pixel_ptr[7] = color;
        }
        cursor_x += 8;
    }
}

// 统一采用高性能透明背景画字引擎，取代旧接口
static void draw_provisioning_screen(esp_lcd_panel_handle_t panel, uint16_t *full_frame)
{
    for (int i = 0; i < LCD_PIXELS; i++) full_frame[i] = 0x0000;
    char pop_str[32]; snprintf(pop_str, sizeof(pop_str), "PoP Key : %s", g_prov_pop);
...
{
    for (int i = 0; i < LCD_PIXELS; i++) full_frame[i] = 0x1F00;
                    ...
                    if (pixel_row & (1 << (7 - col)))
                    {
                        for (int dy = 0; dy < scale; dy++)
                        {
                            for (int dx = 0; dx < scale; dx++)
                            {
                                ...
                                if (px < LCD_WIDTH && py < LCD_HEIGHT) full_frame[py * LCD_WIDTH + px] = 0xFFFF;
                            }
                        }
                    }
                }
...
typedef struct { uint8_t *buf_ptr; uint32_t len; } rx_frame_t;
...
static void sys_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_base == NETWORK_PROV_EVENT)
    {
        switch (event_id)
        {
        case NETWORK_PROV_START: g_is_provisioning = true; break;
        case NETWORK_PROV_WIFI_CRED_FAIL: network_prov_mgr_reset_wifi_sm_state_on_failure(); break;
        case NETWORK_PROV_END: g_is_provisioning = false; network_prov_mgr_deinit(); break;
        default: break;
        }
    }
    ...
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED)
    {
        ...
        if (g_is_provisioning) esp_wifi_connect();
        else {
            if (s_retry_num < 15) { s_retry_num++; esp_wifi_connect(); }
            else {
...
static void load_settings_from_nvs()
{
    ...
    {
        int32_t saved_br = 60;
        if (nvs_get_i32(my_handle, "brightness", &saved_br) == ESP_OK) g_current_brightness = saved_br;

        int8_t saved_osd = 1;
        if (nvs_get_i8(my_handle, "show_osd", &saved_osd) == ESP_OK) g_show_osd = saved_osd;

        int8_t saved_time_osd = 1;
        if (nvs_get_i8(my_handle, "show_time", &saved_time_osd) == ESP_OK) g_show_time_osd = saved_time_osd;

        size_t len_tz = sizeof(g_timezone);
        if (nvs_get_str(my_handle, "tz_str", g_timezone, &len_tz) != ESP_OK) strcpy(g_timezone, "CST-8");

        size_t len = sizeof(g_admin_user);
        if (nvs_get_str(my_handle, "admin_user", g_admin_user, &len) != ESP_OK) strcpy(g_admin_user, "admin");
        
        len = sizeof(g_admin_pwd);
        if (nvs_get_str(my_handle, "admin_pwd", g_admin_pwd, &len) != ESP_OK) strcpy(g_admin_pwd, "123456");

        int32_t saved_port = 8888;
        if (nvs_get_i32(my_handle, "udp_port", &saved_port) == ESP_OK) g_udp_port = saved_port;
...
{
    nvs_handle_t my_handle;
    if (nvs_open("storage", NVS_READWRITE, &my_handle) == ESP_OK) { nvs_set_i32(my_handle, key, val); nvs_commit(my_handle); nvs_close(my_handle); }
}
static void save_i8_to_nvs(const char *key, int8_t val)
{
    nvs_handle_t my_handle;
    if (nvs_open("storage", NVS_READWRITE, &my_handle) == ESP_OK) { nvs_set_i8(my_handle, key, val); nvs_commit(my_handle); nvs_close(my_handle); }
}
static void save_str_to_nvs(const char *key, const char *val)
{
    nvs_handle_t my_handle;
    if (nvs_open("storage", NVS_READWRITE, &my_handle) == ESP_OK) { nvs_set_str(my_handle, key, val); nvs_commit(my_handle); nvs_close(my_handle); }
}
...
static void lcd_hardware_init(void)
{
    ...
    gpio_set_drive_capability(TFT_SCK, GPIO_DRIVE_CAP_3); gpio_set_drive_capability(TFT_MOSI, GPIO_DRIVE_CAP_3);
    gpio_set_drive_capability(TFT_CS, GPIO_DRIVE_CAP_3); gpio_set_drive_capability(TFT_DC, GPIO_DRIVE_CAP_3);
    ...
    uint8_t madctl_val = 0x28; esp_lcd_panel_io_tx_param(io_handle, 0x36, &madctl_val, 1);
...
void jpeg_decode_display_task(void *pvParameters)
{
    ...
    int fps_counter = 0; int current_fps = 0; int8_t current_rssi = 0; int screen_state = 0;
    ...
    while (1)
    {
        ...
        if (xQueueReceive(jpeg_queue, &current_frame, pdMS_TO_TICKS(10)) == pdTRUE)
        {
            ...
            last_frame_time = xTaskGetTickCount(); screen_state = 1;
            ...
            jpeg_dec_io_t jpeg_io = {0}; jpeg_dec_header_info_t out_info = {0};
            jpeg_io.inbuf = current_frame.buf_ptr; jpeg_io.inbuf_len = current_frame.len;
            ...
            if (jpeg_dec_parse_header(jpeg_dec, &jpeg_io, &out_info) == JPEG_ERR_OK)
            {
                ...
                while (cur_row < out_info.height) {
                    ...
                    jpeg_io.inbuf = current_frame.buf_ptr + inbuf_consumed;
                ...
                if (decode_ok) {
                    fps_counter++; TickType_t now = xTaskGetTickCount();
                    
                    static char cached_osd_str[32] = {0}, cached_time_str[32] = {0}, cached_heap_str[32] = {0}, cached_cpu_str[32] = {0};
                    static int cached_time_width = 0;
                    
                    if (now - last_fps_time >= pdMS_TO_TICKS(1000)) {
                        current_fps = fps_counter; fps_counter = 0; last_fps_time = now;
                        g_current_fps = current_fps;

                        uint32_t max_idle_1s_c0 = g_core0_max_idle * 5;
                        uint32_t max_idle_1s_c1 = g_core1_max_idle * 5;
                        
                        uint32_t idle_c0 = g_core0_idle_cnt; g_core0_idle_cnt = 0;
                        uint32_t idle_c1 = g_core1_idle_cnt; g_core1_idle_cnt = 0;
                        ...
                        g_core0_usage = cpu0; g_core1_usage = cpu1;
                        ...
                        time_t now_time; struct tm timeinfo; time(&now_time); localtime_r(&now_time, &timeinfo);
        ...
        {
            if (screen_state == 1 && xTaskGetTickCount() - last_frame_time > pdMS_TO_TICKS(5000)) screen_state = 0;
        ...
        if (screen_state == 0)
        {
            ...
            { 
                if (g_is_provisioning) draw_provisioning_screen(panel_handle, (uint16_t *)display_buf[current_buffer_idx]);
                else draw_no_signal_screen(panel_handle, (uint16_t *)display_buf[current_buffer_idx]);
                current_buffer_idx = (current_buffer_idx + 1) % DISPLAY_BUF_NUM;
            }
            last_screen_state = 0;
        }
            ...
            if (len > 0) {
                ...
                if (is_receiving_frame) {
                    ...
                    if (current_frame_len + 1500 > FRAME_BUF_SIZE) {
                        is_receiving_frame = false; current_frame_len = 0;
                        continue;
                    }
...
static bool is_authenticated(httpd_req_t *req) {
    ...
    if (httpd_req_get_hdr_value_str(req, "Cookie", cookie, sizeof(cookie)) == ESP_OK) {
        if (strstr(cookie, "session=esp32_admin") != NULL) return true;
    }
    return false;
}
...
static esp_err_t do_restart_handler(httpd_req_t *req) {
    if (!is_authenticated(req)) { httpd_resp_set_status(req, "401 Unauthorized"); httpd_resp_send(req, "Unauthorized", HTTPD_RESP_USE_STRLEN); return ESP_FAIL; }
...
static esp_err_t api_ota_handler(httpd_req_t *req) {
    if (!is_authenticated(req)) { httpd_resp_set_status(req, "401 Unauthorized"); httpd_resp_send(req, "Unauthorized", HTTPD_RESP_USE_STRLEN); return ESP_FAIL; }
    esp_ota_handle_t update_handle = 0; const esp_partition_t *update_partition = esp_ota_get_next_update_partition(NULL);
    if (esp_ota_begin(update_partition, OTA_SIZE_UNKNOWN, &update_handle) != ESP_OK) { httpd_resp_send_500(req); return ESP_FAIL; }
    ...
    esp_ota_end(update_handle); esp_ota_set_boot_partition(update_partition);
...
static esp_err_t api_data_handler(httpd_req_t *req) {
    if (!is_authenticated(req)) { httpd_resp_set_status(req, "401 Unauthorized"); httpd_resp_send(req, "Unauthorized", HTTPD_RESP_USE_STRLEN); return ESP_OK; }
    char current_ssid[33] = "Not Connected";
    wifi_config_t wifi_cfg;
    if (esp_wifi_get_config(WIFI_IF_STA, &wifi_cfg) == ESP_OK) {
        if (wifi_cfg.sta.ssid[0] != '\0') strncpy(current_ssid, (char *)wifi_cfg.sta.ssid, sizeof(current_ssid) - 1);
    }
    char json_resp[500];
...
static esp_err_t api_set_handler(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
        ...
        if (httpd_query_key_value(buf, "brightness", param, sizeof(param)) == ESP_OK) {
            int32_t br = atoi(param); if (br < 0) br = 0; if (br > 100) br = 100;
            g_current_brightness = br; save_int_to_nvs("brightness", br);
        ...
        if (httpd_query_key_value(buf, "osd", param, sizeof(param)) == ESP_OK) {
            g_show_osd = (atoi(param) == 1) ? 1 : 0; save_i8_to_nvs("show_osd", g_show_osd);
        }
        if (httpd_query_key_value(buf, "time", param, sizeof(param)) == ESP_OK) {
            g_show_time_osd = (atoi(param) == 1) ? 1 : 0; save_i8_to_nvs("show_time", g_show_time_osd);
        }
        if (httpd_query_key_value(buf, "timezone", param, sizeof(param)) == ESP_OK) {
            ...
            setenv("TZ", g_timezone, 1); tzset(); save_str_to_nvs("tz_str", g_timezone);
        }

        if (httpd_query_key_value(buf, "udp_port", param, sizeof(param)) == ESP_OK) {
            int32_t port = atoi(param);
            if (g_udp_port != port && port > 0 && port < 65536) { g_udp_port = port; save_int_to_nvs("udp_port", port); need_restart = true; }
        }
    }
...
static esp_err_t api_account_handler(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, buf, sizeof(buf)) == ESP_OK) {
        ...
        if (httpd_query_key_value(buf, "user", new_user_enc, sizeof(new_user_enc)) == ESP_OK &&
            ...
            save_str_to_nvs("admin_user", g_admin_user); save_str_to_nvs("admin_pwd", g_admin_pwd);
        }
    }
    httpd_resp_send(req, "OK", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
...
static httpd_handle_t start_webserver(void) {
    httpd_config_t config = HTTPD_DEFAULT_CONFIG(); config.server_port = 80; config.max_uri_handlers = 16;
    httpd_handle_t server = NULL;
    if (httpd_start(&server, &config) == ESP_OK) {
        httpd_uri_t index_uri = {.uri = "/", .method = HTTP_GET, .handler = index_get_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &index_uri);
        httpd_uri_t login_uri = {.uri = "/login", .method = HTTP_GET, .handler = login_get_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &login_uri);
        httpd_uri_t do_login_uri = {.uri = "/do_login", .method = HTTP_GET, .handler = do_login_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &do_login_uri);
        httpd_uri_t api_data_uri = {.uri = "/api/data", .method = HTTP_GET, .handler = api_data_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &api_data_uri);
        httpd_uri_t api_set_uri = {.uri = "/api/set", .method = HTTP_GET, .handler = api_set_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &api_set_uri);
        httpd_uri_t api_acc_uri = {.uri = "/api/account", .method = HTTP_GET, .handler = api_account_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &api_acc_uri);
        httpd_uri_t api_wifi_reset_uri = {.uri = "/api/wifi_reset", .method = HTTP_POST, .handler = api_wifi_reset_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &api_wifi_reset_uri);
        httpd_uri_t ota_uri = {.uri = "/api/ota", .method = HTTP_POST, .handler = api_ota_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &ota_uri);
        httpd_uri_t do_restart_uri = {.uri = "/api/restart", .method = HTTP_POST, .handler = do_restart_handler, .user_ctx = NULL}; httpd_register_uri_handler(server, &do_restart_uri);
    }
    return server;
}
...
void init_time_sync(void) {
    ...
    esp_sntp_setservername(0, "pool.ntp.org"); esp_sntp_setservername(1, "time.stdtime.gov.tw"); esp_sntp_setservername(2, "time.windows.com");
    sntp_set_time_sync_notification_cb(time_sync_notification_cb);
    esp_sntp_servermode_dhcp(1); esp_sntp_init();
    setenv("TZ", g_timezone, 1); tzset();
}
...
void app_main(void) {
    ...
    if (g_core0_max_idle == 0) g_core0_max_idle = 1;
    if (g_core1_max_idle == 0) g_core1_max_idle = 1;

    // 完美優化 1：輸入端緩衝區使用硬體對齊分配 API，徹底滿足 SIMD 向量運算胃口
    for (int i = 0; i < RX_BUF_NUM; i++) {
        rx_frame_buf[i] = (uint8_t *)heap_caps_aligned_alloc(16, FRAME_BUF_SIZE, MALLOC_CAP_SPIRAM);
        if (!rx_frame_buf[i]) while (1) vTaskDelay(1000);
    }

    // 完美優化 2：輸出端顯存緩衝區全面重构升级，改为硬體級對齊分配，徹底拔除偏移隱患
    for (int i = 0; i < DISPLAY_BUF_NUM; i++) {
        display_buf[i] = (uint8_t *)heap_caps_aligned_alloc(16, LCD_PIXELS * 2, MALLOC_CAP_SPIRAM);
    ...
    bool provisioned = false; ESP_ERROR_CHECK(network_prov_mgr_is_wifi_provisioned(&provisioned));
