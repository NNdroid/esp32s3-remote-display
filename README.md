...
A lightweight and efficient remote display solution built on the **ES3C28P** (ESP32-S3) development board. This project allows you to stream video directly to the LCD screen using MJPEG via FFmpeg. 
...
## ✨ Key Features
* **MJPEG Video Streaming**: Decodes and displays real-time MJPEG streams over UDP.
...
```bash
ffmpeg -rtsp_transport tcp -i "rtsp://YOUR_CAMERA_IP/stream" -vf "fps=20,scale=320:240" -an -c:v mjpeg -q:v 3 -pix_fmt yuvj420p -f mjpeg "udp://YOUR_ESP32_IP:8888?pkt_size=1200&buffer_size=65535"